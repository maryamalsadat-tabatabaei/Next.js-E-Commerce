...
interface ProductsProps {
  leastPopularProducts: Product[];
  //   productsCount: number;
  //   numberPerPage: number;
}
const LeastPopularProducts = ({ data }: { data: ProductsProps }) => {
  ...
  return (
    <section className="container max-w-screen-xl mx-auto px-4">
      <div className="grid md:grid-cols-2 gap-2">
        {data?.leastPopularProducts?.length !== 0 ? (
          data?.leastPopularProducts?.map((product) => (
...
export default LeastPopularProducts;
